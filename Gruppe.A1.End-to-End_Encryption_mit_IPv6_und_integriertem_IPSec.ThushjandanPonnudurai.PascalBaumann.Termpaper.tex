...
\subsection{ISAKMP Security Associations}
\label{ssec:SA}
As defined in \citetitle{Harkins1998}: "A security association (SA) is a set of policy and key(s) used to protect information. The ISAKMP SA is the shared policy and key(s) used by the negotiating peers in this protocol to protect their communication." \parencite{Harkins1998}

In short, an SA defines a relationship between two IPsec peers and specifies which options of the ISAKMP and underlying protocols will be used in traffic between these partners.
Some of the concepts used while establishing SAs have been discussed in the preceding chapters (see \ref{ssec:AlgoDH} for an example about hashing).

Sample SA proposal using the IKE key exchange (taken from figure \ref{fig:FirstSAProposal} and abbreviated) in pseudocode:
\begin{lstlisting}
    Type Payload: SA Proposal
    Proposal number: 1
    IKE Encryption Algorithm:   AES-CBC
    IKE Key Length:             256
    IKE Hash Algorithm:         SHA2-256
    IKE Group-Description:      2048bit MODP group (DH group 14)
    IKE Authentication Method:  Pre-shared key
    IKE Life-time:              28800seconds (8 hours)
\end{lstlisting}
