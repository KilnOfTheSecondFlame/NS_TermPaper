...
According to chapter \ref{sec:IPv6IPsec} it should be possible to encrypt easily the traffic between two hosts, because IPsec is a component of IPv6 protocol.

Firstly we have to automate the certificate enrollment process. Unfortunately it is not easily possible to realize this task with OpenSSL as CA. 
Therefore Microsoft Active Directory Certificate Services (ADCS) will be a better choice than OpenSSL. 
...
With SCEP it is possible to enroll automatically certificates for network devices and also for Linux machines. 
Network Device Enrollment Service (NDES) is a module of ADCS. It implements the SCEP protocol. 

Now how we should deploy the IPsec configuration to clients? With group policies we could easily enroll certificates and configure IPsec connection profiles for Windows machines. 
...
There are existing a couple of SCEP clients for Linux. StrongSwan is delivering a client named "ipsec\_scepclient" \parencite{Hutter2005} or CertNanny \parencite{Bartosch2017}, 
which uses the SSCEP Tool from Jarkko Turkulainen, is also a good candidate for certificate autoenrollment.

Combining these given tools it is possible to build up automated End-to-End encryption as well for IPv6-only Linux environments.

\section{Conclusion}
\label{sec:Conc}
In section \ref{sec:ExpectedResult} we adjusted to have difficulties configuring IPsec on a Windows computer. But the contrary has been happened during the experiment. 
We had more difficulties to configure IPsec VPN on Linux than on Windows. Windows doesn't support a lot of encryption and hashing algorithm, but nevertheless it was easily to find the matching algorithm for both clients.

End-to-End encryption can be in a Windows IPv6-only network easily deployed. Microsoft is delivering already suitable tools for this use case. 
If Linux machines are also a part of the network, then several tools needs to be combined to achieve this goal. Thus it is possible to realize an End-to-End encryption between managed-machines in a network. 
We have investigated only solutions with managed machines. It could be possible to offer End-to-End encryption for unmanaged clients, but this was not the scope of our term paper

Unfortunately we have lost a lot of time for troubleshooting simple VPN issues. That's why we couldn't investigate more in this topic. 
But we hope that this term paper gives someone a good fundament for further researches. 
