...
\end{itemize}
AES-256 is commonly used in VPNs. DES is not secure anymore, because the DES key can be cracked in less than 24 hours.

\subsubsection{Asymmetric}
\label{sssec:asymmetric}
Asymmetrical algorithms, also known as public key algorithm, use separate keys for encryption and decryption. The public key is used for the encryption and can be made public. The private key is used for decryption and must be kept secret. The public key and the private key are mathematically related, it is not possible to find out the private key from the public key. 

RSA, DSA and Elliptic Curve (ECC) are some asymmetrical algorithm.

\subsubsection{Diffie-Hellman (DH)}
\label{sssec:diffie-hellman}
Diffie-Hellman algorithm (DH) allows two devices to establish a shared secret over an unsecure network. It was published by Whitfield Diffie and Martin Hellman in 1976. Related to VPN, it is used in the IKE or Phase 1 for key exchange. 
...
Message-Digest-Algorithm 5 (MD5), Secure Hash Algorithm 1 (SHA-1) or SHA-2 are some example of hashing algorith. MD5 and SHA-1 are \textbf{insecure} and should not be used. 
